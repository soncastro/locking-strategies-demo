...
export default function () {
  const debitUrl1 = `${BASE_URL}/debit-synchronized/${ACCOUNT_ID}/${DEBIT_AMOUNT_1}`;
  const debitUrl2 = `${BASE_URL}/debit-synchronized/${ACCOUNT_ID}/${DEBIT_AMOUNT_2}`;

  const responses = http.batch([
    ['POST', debitUrl1, null, { tags: { request_name: 'debit_1' } }],
    ['POST', debitUrl2, null, { tags: { request_name: 'debit_2' } }],
  ]);

  responses.forEach((response, index) => {
    const requestName = `debit_${index + 1}`;
    console.log(
      [
        `\n[${requestName}]`,
