...
### `POST /locking-strategies-demo/reset`

Restaura o estado base da demonstração:

- apaga aferições
- recria a aferição inicial do pneu `1` com vida `2`
- remove movimentações da conta `1`
- redefine o saldo da conta `1` para `100`

### `POST /locking-strategies-demo/debit/{accountId}/{amount}`

Executa um débito lendo o saldo atual, aguardando `1` segundo para simular processamento e depois atualizando o saldo. Como não há proteção específica de concorrência, duas requisições simultâneas podem ler o mesmo saldo e produzir efeito incorreto.

### `POST /locking-strategies-demo/debit-synchronized/{accountId}/{amount}`

Executa o mesmo débito, mas protegido por `synchronized (ExamplesController.class)`. Isso serializa as requisições dentro da instância da aplicação e evita concorrência entre threads no mesmo processo.

### `POST /locking-strategies-demo/debit-pessimistic-locking/{accountId}/{amount}`

Executa o débito adquirindo lock no registro da conta com `FOR UPDATE` antes de ler e atualizar o saldo. O banco passa a coordenar a concorrência entre transações.

### `POST /locking-strategies-demo/debit-optimistic-locking/{accountId}/{amount}`

Executa o débito usando o campo de versão da conta. A atualização só acontece se a versão lida inicialmente ainda for a mesma no momento do `UPDATE`. Em caso de conflito concorrente, a operação falha com erro de lock otimista.

### `POST /locking-strategies-demo/cenario-ficticio-afericao-pneu-com-inconsistencia/{pneuId}/{vidaAfericao}`

Lê a última vida registrada para o pneu e insere uma nova aferição apenas se a nova vida não for menor que a última. Como não existe lock protegendo a leitura e a gravação, duas requisições concorrentes podem passar na validação e gravar dados inconsistentes.

### `POST /locking-strategies-demo/cenario-ficticio-afericao-pneu-sem-inconsistencia/{pneuId}/{vidaAfericao}`

Executa o mesmo cenário, mas antes faz lock no registro do pneu com `FOR UPDATE`. Assim, a verificação da última vida e a inserção da nova aferição acontecem de forma protegida contra concorrência.
