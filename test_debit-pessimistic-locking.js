...
export default function () {
  const debitUrl1 = `${BASE_URL}/debit-pessimistic-locking/${ACCOUNT_ID}/${DEBIT_AMOUNT_1}`;
  const debitUrl2 = `${BASE_URL}/debit-pessimistic-locking/${ACCOUNT_ID}/${DEBIT_AMOUNT_2}`;

  const responses = http.batch([
    ['POST', debitUrl1, null, { tags: { request_name: 'debit_pessimistic_locking_1' } }],
    ['POST', debitUrl2, null, { tags: { request_name: 'debit_pessimistic_locking_2' } }],
  ]);

  responses.forEach((response, index) => {
    const requestName = `debit_pessimistic_locking_${index + 1}`;
    console.log(
      [
        `\n[${requestName}]`,
